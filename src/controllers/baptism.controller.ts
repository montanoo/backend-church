...
import { generateBaptismPDF } from "../pdf/generateBaptismPDF";
...
const getAllBaptisms = async (
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "Failed to retrieve baptisms", error });
  }
};

const getBaptismById = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Failed to retrieve baptism", error });
  }
};
...
const updateBaptism = async (
  ...
  try {
    const { id } = req.params;
    const updatedBaptism = await baptismService.updateBaptism(
      parseInt(id),
      req.body
    );
...
export const getBaptismCertificatePDF = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const baptism = await baptismService.getBaptismById(parseInt(id));
    ...
    const pdfBuffer = await generateBaptismPDF();

    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", "inline; filename=baptism.pdf");

    res.end(pdfBuffer); // ✅ USA res.end(buffer) para binarios
  } catch (error) {
